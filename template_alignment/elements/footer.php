
<div class="footer">

<div class="container">



<div class="row">
            <div class="col-lg-3 col-sm-3">
                   <h4>Information</h4>
                   <ul class="row">
                <li class="col-lg-12 col-sm-12 col-xs-3"><a href="about_us.php">About</a></li>
                <!-- <li class="col-lg-12 col-sm-12 col-xs-3"><a href="#">Agents</a></li>         
                <li class="col-lg-12 col-sm-12 col-xs-3"><a href="#">Blog</a></li> -->
                <li class="col-lg-12 col-sm-12 col-xs-3"><a href="contact_us.php">Contact</a></li>
              </ul>
            </div>
            
            <div class="col-lg-3 col-sm-3">
                    <h4>Newsletter</h4>
                    <p>Get notified about the latest properties in our marketplace.</p>
                    <form class="form-inline" role="form">
                            <input type="text" placeholder="Enter Your email address" class="form-control">
                                <button class="btn btn-success" type="button">Notify Me!</button></form>
            </div>
            
            <div class="col-lg-3 col-sm-3">
                    <h4>Follow us</h4>
                    <a href="index.php#"><img src="./images/facebook.png" alt="facebook"></a>
                    <a href="index.php#"><img src="./images/twitter.png" alt="twitter"></a>
                    <a href="index.php#"><img src="./images/linkedin.png" alt="linkedin"></a>
                    <a href="index.php#"><img src="./images/instagram.png" alt="instagram"></a>
            </div>

             <div class="col-lg-3 col-sm-3">
                    <h4>Contact us</h4>
                    <p><b>Bootstrap Realestate Inc.</b><br>
<span class="glyphicon glyphicon-map-marker"></span> 8290 Walk Street, Australia <br>
<span class="glyphicon glyphicon-envelope"></span> hello@bootstrapreal.com<br>
<span class="glyphicon glyphicon-earphone"></span> (123) 456-7890</p>
            </div>
        </div>
<p class="copyright">Copyright 2013. All rights reserved.	</p>


</div></div>